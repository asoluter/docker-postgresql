# asoluter/postgresql:14-20220828
...
```bash
docker pull asoluter/postgresql:14-20220828
```
...
```bash
docker run --name postgresql -itd --restart always \
  asoluter/postgresql:14-20220828 -c log_connections=on
```
...
```bash
docker run --name postgresql -itd --restart always \
  asoluter/postgresql:14-20220828 -c logging_collector=on
```
